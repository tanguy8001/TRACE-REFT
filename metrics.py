...
# compatibility layer for metrics loading (datasets.load_metric deprecated -> evaluate.load)
try:
    import evaluate  # type: ignore
    _load_metric = evaluate.load
except Exception:
    try:
        from datasets import load_metric as _load_metric  # type: ignore
    except Exception:
        _load_metric = None
...
def caculate_sari(inputs, results, data):
    if _load_metric is None:
        raise ImportError("No metric loader available. Please install 'evaluate' (pip install evaluate).")
    sari = _load_metric("sari")
    res = sari.compute(sources=inputs, predictions=results, references=[[label] for label in data])
    if isinstance(res, dict) and "sari" in res:
        return res["sari"]
    return res
