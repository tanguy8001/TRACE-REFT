...
from templates import alpaca_prompt_no_input_template
from loreft_datasets import LoReftGLUEDataset, LoReftSupervisedDataset, parse_positions
...
def finetune(
    ...
    is_local_model_path = os.path.isdir(model)
    if layers.strip() == "":
        layers = []
    elif layers != "all":
        layers = [int(l) for l in layers.split(";")]
    else:
        temp_config = AutoConfig.from_pretrained(model, local_files_only=is_local_model_path)
    ...
    if is_wandb:
        ...
        wandb.log({"train/n_params": n_params, "train/n_params_with_model": n_params_with_model})
    ...
    training_kwargs = {
        ...
        "report_to": ["wandb"] if is_wandb else [],
        "seed": seed,
    ...
    # Filter by available fields to avoid unexpected keyword errors
    valid_fields = set(TrainingArguments.__dataclass_fields__.keys())
    filtered_kwargs = {k: v for k, v in training_kwargs.items() if k in valid_fields}
    ...
    trainer = trainer_class(
        ...
        eval_dataset=in_train_eval_datasets if (task == "glue" and not no_eval) else None,
    ...
    if task not in classification_tasks:
        ...
        front_positions = list(range(min(first_n, seq_len))) if first_n > 0 else []
        back_positions = list(range(max(0, seq_len - last_n), seq_len)) if last_n > 0 else []
        selected_positions = sorted(set(front_positions + back_positions)) or [max(0, seq_len - 1)]

        num_interventions = len(layers) if isinstance(layers, list) else len([int(l) for l in layers.split(";")])
        unit_locations = [[selected_positions] for _ in range(num_interventions)]
        ...
        _, sanity_outputs = reft_model.generate(**generation_args)
        sanity_text = tokenizer.batch_decode(sanity_outputs, skip_special_tokens=True)[0]
        print("SANITY_GENERATION:", sanity_text)
    ...
    for _, intervention in reft_model.interventions.items():
        if isinstance(intervention, (list, tuple)):
            for item in intervention:
                if hasattr(item, "eval"):
                    item.eval()
        elif hasattr(intervention, "eval"):
            intervention.eval()
...
def main():
    ...
    parser.add_argument('-no_eval', '--no_eval', action='store_true')
